...
void detectObjects(cv::Mat &img, std::vector<BoundingBox> &bBoxes, const float confThreshold,
    ...
    std::vector<int> outLayers{ net.getUnconnectedOutLayers() }; // get  indices of  output layers, i.e.  layers
    ...
    for (size_t i{ 0 }; i < netOutput.size(); ++i) {
        ...
        float *data{ reinterpret_cast<float*>(netOutput[i].data) };
    ...
    if (bVis) {
        ...
        for (auto it{ std::begin(bBoxes) }; it != std::end(bBoxes); ++it) {
